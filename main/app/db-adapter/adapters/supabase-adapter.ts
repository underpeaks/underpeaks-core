...
export class SupabaseAdapter implements DBAdapter {
  ...
  constructor(config: DBConfig) {
    ...
    const serviceRoleKey = config.anonKey;
    if (!serviceRoleKey) throw new Error('Supabase service key (anonKey) is required in config');
...
//     // If the error is about conflict or already exists, try to fetch the user instead
//     if (error.message.includes('duplicate') || error.message.includes('already exists')) {
//       const existing = await this.findUserByEmailWithRetry(config, user_email, retries, delayMs);
...
//   throw new Error(`Failed to create or retrieve admin user after ${retries} attempts`);
// }
   ...
   * BUILT-IN AUTH
   * ----------------------------- */
  async signUpWithEmailPassword(config: DBConfig, email: string, password: string, fullName?: string) {
    return this.registerUserInAuth(config, { email, password, fullName });
  }

  async login() {
    return {
      error:
        'login() via Supabase server SDK not supported. Use client SDK and pass ID token to validateBuiltInSession().'
    };
  }
    ...
    const columnsSql = columnsArray
      .map((col) => {
        ...
        switch (col.type) {
          ...
          case 'timestamptz': typeSql = 'TIMESTAMPTZ'; if (col.default === 'now()') defaultValue = ' DEFAULT now()'; break;
...
// this.adminClient = createClient(config.url, serviceRoleKey);
//   }
//   supportsBuiltInAuth?: boolean | undefined;
//   register?(config: DBConfig, data: { email: string; password: string; full_name?: string; }): Promise<{ userId?: string; uid?: string; error?: string; }> {
...
//     throw new Error('Method not implemented.');
//   }
//   sendResetEmail?(config: DBConfig, email: string, redirectUrl: string): Promise<{ success: boolean; error?: string; }> {
//     throw new Error('Method not implemented.');
//   }
//   resetPassword?(config: DBConfig, token: string, newPassword: string): Promise<{ success: boolean; error?: string; }> {
//     throw new Error('Method not implemented.');
//   }
//   signInWithEmailPassword?(config: DBConfig, email: string, password: string): Promise<any> {
//     throw new Error('Method not implemented.');
//   }
//   signUpWithEmailPassword?(config: DBConfig, email: string, password: string, fullName?: string): Promise<any> {
//     throw new Error('Method not implemented.');
//   }
