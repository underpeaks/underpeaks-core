...
const DATABASES = [
  ...
  //   value: 'mariadb',
  //   name: 'MariaDB',
  //   description: 'MySQL-compatible, often faster.',
  //   icon: <SiMariadb className="w-5 h-5 text-red-600" />,
  //   fields: [
  //     { key: 'host', label: 'Host', placeholder: 'localhost' },
  //     { key: 'port', label: 'Port', placeholder: '3306' },
  //     { key: 'database', label: 'Database Name', placeholder: 'mydb' },
  ...
  //   icon: <SiPlanetscale className="w-5 h-5 text-indigo-600" />,
  //   fields: [
  //     { key: 'host', label: 'Host', placeholder: 'us-east.connect.psdb.cloud' },
  //     { key: 'port', label: 'Port', placeholder: '3306' },
  //     { key: 'database', label: 'Database Name', placeholder: 'mydb' },
  //     { key: 'user', label: 'Username', placeholder: 'username' },
  //     { key: 'password', label: 'Password', placeholder: 'yourpassword' },
  //     { key: 'sslCert', label: 'SSL Certificate (optional)', placeholder: '-----BEGIN CERTIFICATE-----...' },
  //   ],
  // },
]
...
export default function DatabaseConfigPage() {
  ...
  const setInstallerValue = useInstallerStore((s: { setInstallerValue: any }) => s.setInstallerValue)
  ...
  const handleContinue = async () => {
    setLoading(true)
    try {
      setInstallerValue('selectedDb', selectedDb as any)
      setInstallerValue('dbConfig', formData)
      setTimeout(() => {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const saveResponse = await fetch('/api/save-db-config', {
        method: 'POST',
        body: JSON.stringify({ type: selectedDb, ...formData }),
      })
      ...
      <div className="w-full max-w-xl p-8 bg-gray-50 rounded-2xl shadow-xl border space-y-6">
        ...
        {selectedDb === 'mongodb' && (
          <div className="mb-6 p-4 bg-green-50 border-l-4 border-green-400 text-green-700 text-sm">
            ...
            <p className="mt-1">
              If your password contains special characters (e.g. <code>@</code>, <code>%</code>, <code>/</code>),
              they must be URL-encoded. For example, <code>@</code> becomes <code>%40</code>.
            </p>
